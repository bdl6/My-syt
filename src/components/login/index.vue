          ...
          <div class="login">
            <div v-show="scene == 0">
              <el-form :model="Param" :rules="rules">
                ...
                <el-form-item>
                  <el-button :disabled="!isPhone" type="primary" size="default">
                    <CountDown
              ...
              <el-button
                ...
                type="primary"
                size="default"
                :disabled="!isPhone || Param.code.length < 6"
                @click="login"
                >用户登录</el-button
              >
            ...
            <div class="webchat" v-show="scene == 1">
              ...
              <div class="phone" @click="changeScene(0)">
                <p>手机短信验证码登录</p>
                <svg
                  t="1685676069573"
                  class="icon"
...
const countStart = ref<boolean>(false)
const countEnd = (val: boolean) => {
  countStart.value = val
}
...
const isPhone = computed(() => {
  ...
  return phoneReg.test(Param.phone)
})

const getCode = async () => {
  try {
    //校验手机号和是否在倒计时
    if (!isPhone.value || countStart.value) {
      return
    }
    countStart.value = true //倒计时开始
    await userStore.getCode(Param.phone)
    Param.code = userStore.code
  } catch (error) {
    ElMessage({
      type: 'error',
...
const scene = ref<number>(0) // 0:手机号登录 1:微信扫码登录

const changeScene = async (mode: number) => {
  scene.value = mode
  //生成二维码
